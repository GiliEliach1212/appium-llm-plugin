...
export function userXmlAndImageQueryPrompt(xml, screenshotDescription, elementDescription) {
  return `
[USERXML]${xml}[/USERXML]
[SCREENSHOT]${screenshotDescription}[/SCREENSHOT]
I am looking for the following element: [ELDESC]${elementDescription}[/ELDESC]

Use both the XML hierarchy and the screenshot description to find the most accurate match.
Respond ONLY with a single XPath selector, in JSON format with the key 'xpath', and nothing else.
If uncertain, choose the element that is most likely correct according to both sources.`
}

export function userXmlThenImageVerifyPrompt(xml, elementDescription, screenshotDescription) {
  return `
[USERXML]${xml}[/USERXML]
Find the following element: [ELDESC]${elementDescription}[/ELDESC]

Once you have found the element in the XML, verify using the screenshot description ([SCREENSHOT]${screenshotDescription}[/SCREENSHOT]) that it is indeed the correct element.
Respond with a single XPath selector in JSON format with the key 'xpath'. If the element is not found or does not match the screenshot, return {"xpath": null}.`
}
