...
import {SYSTEM_SETUP_PROMPT_IMAGE, SYSTEM_SETUP_PROMPT_XML, SYSTEM_SETUP_PROMPT_XML_POS, userImageQueryPrompt, userXmlPosQueryPrompt, userXmlQueryPrompt, userXmlAndImageQueryPrompt, userXmlThenImageVerifyPrompt} from './prompts.mjs'
...
export class XmlAndImageQuery extends Query {
  /**
   * @param {number} temperature
   * @param {string} xml
   * @param {string} screenshotDescription
   * @param {string} desc
   * @param {string?} model
   */
  async run(temperature, xml, screenshotDescription, desc, model) {
    const response = await this.client.chat.completions.create({
      temperature,
      model,
      response_format: {type: 'json_object'},
      messages: [{
        ...
        role: 'user',
        content: userXmlAndImageQueryPrompt(xml, screenshotDescription, desc),
      }]
    })
    this.logger.info(`Received response from AI model: ${JSON.stringify(response)}`)
    let xpath
...
export class XmlThenImageVerifyQuery extends Query {
  /**
   * @param {number} temperature
   * @param {string} xml
   * @param {string} desc
   ...
   * @param {string?} model
   */
  async run(temperature, xml, desc, screenshotDescription, model) {
    const response = await this.client.chat.completions.create({
      temperature,
      model,
      response_format: {type: 'json_object'},
      messages: [{
        ...
        role: 'user',
        content: userXmlThenImageVerifyPrompt(xml, desc, screenshotDescription),
      }]
    })
    this.logger.info(`Received response from AI model: ${JSON.stringify(response)}`)
    let xpath
